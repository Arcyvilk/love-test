...
entity.update  = function(self)
  ...
  if input.left.active then
    local boundary_left = math.max(x_self - vars.player_speed, vars.player_radius)
    self.body:setPosition(boundary_left, y_self)
  end

  if input.right.active then
    local boundary_right = math.min(x_self + vars.player_speed, vars.world_width - vars.player_radius)
    self.body:setPosition(boundary_right, y_self)
  end

  if input.up.active then
    local boundary_top = math.max(vars.boundary_offset_top + vars.player_radius, y_self - vars.player_speed)
    self.body:setPosition(x_self, boundary_top)
  end

  if input.down.active then
    local boundary_down = math.min(vars.world_height - vars.player_radius, y_self + vars.player_speed)
